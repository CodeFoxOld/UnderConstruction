...
namespace de.trustfallgames.targetsenior.tilemap {
    ...
    public class MapManager : MonoBehaviour {
        [Header("Use \"Generate Classes for Tiles\" to add scripts to all tiles and fill refresh values")] [SerializeField]
        ...
        public void GenerateTileClasses() {
            foreach(Transform child in transform) {
                ...
                child.transform.transform.position = new Vector3((float)Math.Round(child.transform.position.x, MidpointRounding.ToEven), 0, (float)Math.Round(child.transform.position.z, MidpointRounding.ToEven));
                child.gameObject.GetComponent<Tile>().SetTilecords((int)child.transform.position.x, (int)child.transform.position.z);
            }

            lastRefresh = DateTime.Now.Hour + ":" + DateTime.Now.Minute + ":" + DateTime.Now.Second;
        }

        [Header("User \"Generate Field with Dimensions\" to generate a tilemap with the following dimensions")] [SerializeField]
